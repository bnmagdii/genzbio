...
import { LogIn, ShieldAlert, Key, Eye, EyeOff, Loader2 } from "lucide-react";
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.message || "Credential authentication failed. Access denied.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="flex-1 flex flex-col items-center justify-center p-4 min-h-screen">
      <div className="w-full max-w-md glass-panel-glow p-8 md:p-10 border border-purple-500/30 flex flex-col gap-6 relative overflow-hidden">
          ...
          <div className="flex flex-col gap-1">
            <div className="flex items-center justify-between">
              ...
              <Link
                ...
                className="text-[10px] uppercase font-semibold text-cyan-400 hover:text-accent transition-colors"
              >
                Reset Sector?
              </Link>
            </div>
        ...
        >
          ...
          ACCESS WITH GOOGLE ORBIT
        </button>

        {/* Bottom Link */}
        <div className="text-center text-xs text-gray-400">
          New space sector explorer?{" "}
          <Link href="/signup" className="text-cyan-400 hover:text-accent font-semibold underline transition-colors">
            Initialize Orbit
          </Link>
        </div>
      </div>
